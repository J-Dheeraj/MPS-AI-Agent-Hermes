...
echo ""
warn "IMPORTANT: Edit the two config.yaml files below to add Telegram bot tokens."
warn "  ~/.hermes/profiles/mps-main/config.yaml         (MP bot — safe to enable Telegram)"
warn "  ~/.hermes/profiles/mps-volunteers/config.yaml   (volunteer bot — safe to enable Telegram)"
warn ""
warn "SECURITY: Do NOT add a Telegram token to mps-vetters/config.yaml."
warn "  Vetters process letter content (constituent data). Telegram is a cloud"
warn "  service. Enabling it here would route constituent data off-premises."
warn "  Vetters must use the NanoClaw web UI (http://localhost:3080) instead."
echo ""
...
echo "=================================================="
echo ""
echo "1. Edit the TWO config.yaml files (mps-main and mps-volunteers only) to add"
echo "   your Telegram bot tokens and user IDs. DO NOT add a token to mps-vetters."
echo "   (See security note in profiles/mps-vetters/config.yaml.)"
# original: echo "1. Edit the three config.yaml files to add your Telegram bot tokens and"
